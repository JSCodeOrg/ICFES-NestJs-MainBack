...
export class Resultado {
  ...
  @Prop({ index: true }) COLE_NATURALEZA: string;
  ...
  @Prop({ index: true }) ESTU_GENERO: string;
  ...
  @Prop() PUNT_LECTURA_CRITICA: number;
  @Prop() PUNT_MATEMATICAS: number;
  @Prop() PUNT_SOCIALES_CIUDADANAS: number;
  @Prop() PUNT_C_NATURALES: number;
  @Prop() PUNT_INGLES: number;
  @Prop() DESEMP_INGLES: string;

  @Prop({ index: true }) PUNT_GLOBAL: number;
  @Prop({ index: true }) ANIO_EXAMEN: number;
  @Prop() EDAD: number;
...
ResultadosSchema.index({ ANIO_EXAMEN: 1, ESTU_DEPTO_RESIDE: 1 });